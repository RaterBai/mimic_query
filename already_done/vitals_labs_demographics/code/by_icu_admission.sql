with first_icu_time as 
(
	select subject_id, min(intime) from icustays group by subject_id
), first_icu as 
(
	select icu.subject_id, 
		   ...
		   intime,
		   outtime
	from first_icu_time first
	inner join icustays icu
	on first.subject_id = icu.subject_id and first.min = icu.intime
), last_icu_time as 
(
...
(
	select subject_id, hadm_id, icustay_id, charttime, itemid, value, valuenum from chartevents 
	where itemid in (
	...
	union 
	select subject_id, hadm_id, case when true then 00000 end as icustay_id, charttime, itemid, value, valuenum from labevents 
	...
	select subject_id, hadm_id, charttime, 
		...
		from vitals
) select icu.subject_id, icu.hadm_id, icu.icustay_id, tag.charttime, tag.value, tag.valuenum, tag.tag from tagged_vitals tag
inner join last_icu icu
on tag.hadm_id = icu.hadm_id
where tag is not null and tag.charttime between icu.intime and icu.outtime
...
(
	...
	select subject_id, hadm_id, charttime, valuenum,
		...
		from labevents
) select icu.subject_id, icu.hadm_id, icu.icustay_id, tag.charttime, tag.valuenum, tag.tag from tagged_labevents tag
inner join first_icu icu
on tag.hadm_id = icu.hadm_id
where tag is not null and tag.charttime between icu.intime and icu.outtime
order by subject_id, hadm_id, tag, charttime